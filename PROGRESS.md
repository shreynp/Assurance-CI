...
- 2026-06-24: Richer build context — added 5 new fields to context.json: symbol_signatures (full TS/Python type signatures per changed symbol), file_contents (full source for files ≤200 lines), file_imports (import specifiers declared inside each changed file), file_directives ('use client'/'use server' per TS/TSX file for Next.js component type detection), existing_tests (co-located .test.ts/.spec.tsx/__tests__/ files + previously generated tests from generated/$STORY_ID/). Also fixed find_ts_callers to scan _TS_SEARCH_ROOTS (app/, components/, lib/, hooks/, stores/, pages/) instead of just src/ — the old scan was a no-op for every Next.js project. Skill updated to pass --story-id to build_context.py. 18 context tests green.
- 2026-06-24: RCA in PR report — build_pr_body.py now adds a Root Cause Analysis section when any tests fail. Two layers: (1) static RCA table parsed from pytest --tb=short output (test name, one-line exception, first ≤3 assertion lines); (2) Claude Haiku-generated plain-English summary blockquote explaining likely root cause and recommending a fix. ANTHROPIC_API_KEY exposed to the Build PR body workflow step. Haiku call fails silently — table always renders.
